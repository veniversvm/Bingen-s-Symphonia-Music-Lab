...
import { IntervalGenerator, INTERNAL_MODES, INTERVAL_LEVELS } from "@bingens/core";
...
export const IntervalDictationGame = (props: { settings: IntervalSettings, onExit: () => void }) => {
  ...
  const generateCustomChallenge = () => {
    ...
    const modes = props.settings.playbackModes.length > 0 
  ...
  const getLevelName = () => props.settings.mode === 'mastery' ? INTERVAL_LEVELS[levelIdx()].name : 'Personalizado';
  const getSubLevelName = () => props.settings.mode === 'mastery' ? (t(`sublevels.${INTERNAL_MODES[subLevelIdx()]}` as any) || 'FULL MIX') : 'Aleatorio';
  const getOptions = () => props.settings.mode === 'mastery' ? INTERVAL_LEVELS[levelIdx()].intervals : props.settings.selectedIntervals;
  ...
  return (
    <Show when={!isGameOver()} fallback={
       <ExerciseSummary 
          ...
          onRetry={() => { 
             setSessionTotal(0); setSessionScore(0); setStreak(0); setIsGameOver(false); nextChallenge(); 
          }} 
          onExit={props.onExit} 
       />
    }>
