        ...
        <div class="flex flex-col bg-base-100 rounded-2xl shadow-xl border border-base-content/10 overflow-hidden transition-all">
          ...
          <div class="flex items-center justify-between px-3 py-2 md:px-6 md:py-4 bg-base-200/50 border-b border-base-content/5">
            <div class="flex items-center gap-2">
              ...
              <div class="flex flex-col">
                <span class="text-[9px] md:text-xs font-black uppercase opacity-40">
                  Instrumento
                </span>
            ...
            <div class="flex items-center gap-3 md:gap-6 text-right">
              <div>
                <p class="text-[9px] md:text-xs font-black opacity-40 uppercase tracking-widest leading-none mb-1">
                  Progreso
                </p>
                <p class="text-xs md:text-base font-mono font-bold leading-none">
                  {count()}{" "}
                  <span class="opacity-30 text-[10px]">
          ...
          >
            <Show
              when={feedback()}
              fallback={
                <span class="text-[10px] md:text-xs uppercase font-bold tracking-widest italic opacity-50">
                  Escucha el acorde
                </span>
              }
            >
          ...
          <div class="grid grid-cols-2 border-t border-base-content/10 bg-base-200/30">
            <button
              class="btn btn-ghost rounded-none border-r border-base-content/5 gap-2 h-12 md:h-16"
              onClick={() => challenge() && audioEngine.play(getActiveNotes())}
            >
              <Play size={16} class="fill-current" />{" "}
              <span class="text-[10px] md:text-sm uppercase font-black">
                Acorde
              </span>
            </button>
            <button
              class="btn btn-ghost rounded-none gap-2 h-12 md:h-16"
              onClick={() =>
                challenge() && audioEngine.arpeggiate(getActiveNotes())
              }
            >
              <Music2 size={16} />{" "}
              <span class="text-[10px] md:text-sm uppercase font-black">
          ...
          <div class="card-body p-3 md:p-8 space-y-3 md:space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6">
              ...
              <div class="dropdown w-full">
                <label class="text-[9px] md:text-xs uppercase font-black opacity-40 mb-1 block ml-1 tracking-widest">
                  Calidad
                </label>
                <div
                  tabindex="0"
                  role="button"
                  class={`btn btn-outline btn-sm md:btn-md h-11 md:h-14 w-full justify-between font-bold border-base-content/10 ${feedback() ? "btn-disabled" : ""}`}
                >
                  <span class="truncate md:text-base">
                    {getSelectedTypeName()}
                  </span>
                  <ChevronDown size={14} />
                </div>
                <ul
                  tabindex="0"
                  class="dropdown-content z-[30] menu p-1 shadow-2xl bg-base-100 rounded-xl w-full border border-base-content/10 mt-1 max-h-60 overflow-y-auto"
                >
                  <For
                    each={CHORD_TYPES.filter((t) =>
                      props.settings.types.includes(t.symbol),
                    )}
                  >
                    {(type) => (
                      <li>
                        <button
                          class="py-3 md:text-base font-bold text-left"
                          onClick={() => {
                            (document.activeElement as HTMLElement)?.blur();
                            setSelectedType(type.symbol);
                          }}
                        >
              ...
              <div class="dropdown w-full">
                ...
                <ul
                  tabindex="0"
                  class="dropdown-content z-[30] menu p-1 shadow-2xl bg-base-100 rounded-xl w-full border border-base-content/10 mt-1"
                >
                  <For each={props.settings.inversions}>
                    {(inv) => (
                      <li>
                        <button
                          class="py-3 md:text-base font-bold text-left"
                        ...
                        >
                          {
                            t(
                              `config.inversions_labels.${["fundamental", "first", "second", "third"][inv]}` as any,
                            ) as string
                          }
                        </button>
                      </li>
            ...
            <div class="pt-1">
              <Show
                ...
                fallback={
                  ...
                  >
                    {isLastExercise()
                      ? (t("config.finish" as any) as string)
