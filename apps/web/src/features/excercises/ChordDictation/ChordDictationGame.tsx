...
export const ChordDictationGame = (props: Props) => {
  ...
  return (
    // LA MAGIA DE SOLID: Usamos <Show> para alternar entre el Juego y el Resumen
    <Show 
      when={!isGameOver()} 
      fallback={
        <ExerciseSummary
          score={score()}
          // Si el usuario ya respondió (hay feedback), el total es el conteo actual.
          // Si abandonó antes de responder el actual, el total es el anterior.
          // Math.max(..., 0) asegura que nunca enviamos un número negativo.
      ...
      <div class="w-full max-w-md md:max-w-2xl lg:max-w-3xl mx-auto px-2 space-y-3 md:space-y-6 animate-fade-in pb-10">
        ...
        <div class="flex flex-col bg-base-100 rounded-2xl shadow-xl border border-base-content/10 overflow-hidden">
          ...
          <div class="flex items-center justify-between px-3 py-2 md:px-6 md:py-4 bg-base-200/50 border-b border-base-content/5">
            <div class="flex items-center gap-2">
              <button 
                ...
                class="btn btn-ghost btn-xs md:btn-sm text-error font-black gap-1 p-1 md:px-2 hover:bg-error/10"
              >
                <LogOut size={14} />
                <span class="hidden sm:inline uppercase tracking-tighter">{t('common.finishEarly' as any) || 'Terminar'}</span>
              </button>
            ...
            <div class="flex items-center gap-3 md:gap-6">
              ...
              <div class="w-8 h-8 md:w-12 md:h-12 rounded-full flex items-center justify-center border-2 border-base-content/5">
                 <Show when={feedback() === 'correct'}><CircleCheck class="text-success" size={24} /></Show>
                 <Show when={feedback() === 'wrong'}><CircleX class="text-error" size={24} /></Show>
          ...
          >
            <Show 
              when={feedback()} 
              fallback={<span class="text-[10px] md:text-xs uppercase font-bold tracking-widest italic opacity-50">{t('home.subtitle' as any) || 'Escucha el acorde'}</span>}
            >
            ...
            <div class="pt-1">
              <Show when={!feedback()} fallback={
                <button 
                  ...
                  onClick={handleNext}
                >
                  {isLastExercise() ? (t('common.finish' as any) || 'Finalizar') : (t('common.next' as any) || 'Siguiente')}
